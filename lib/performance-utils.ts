    ...
    try {
      new PerformanceObserver((entryList) => {
        ...
        const lastEntry = entries[
          entries.length - 1
        ] as PerformanceEntryWithElement;
...
const optimizeScriptLoading = () => {
  ...
  const preloadScripts = [
    "/chunks/56-e73f64e63a5098d6.js",
    "/chunks/908-3dc593e90316e7e1.js",
  ];
  ...
  // Defer non-critical scripts
  document
    .querySelectorAll("script:not([async]):not([defer])")
    .forEach((scriptElement) => {
      const script = scriptElement as HTMLScriptElement;
      if (
        script.src &&
        !script.src.includes("chunks/56-") &&
        !script.src.includes("chunks/908-")
      ) {
        script.defer = true;
      }
    });
